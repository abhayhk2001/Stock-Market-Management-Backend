...
function authorizeLogin(username, password, isvalid) {
  ...
  var sql = `select password from investorlogin where username = '${username}'`;
  db.executeQuery(sql,function (err, result) {
    console.log(result);
 ...
 function getPrivateDetails(presult,id,success){
  ...
  db.executeQuery(sql,function(err,result){
    ...
    if(!err && result.length>0){
      presult[0].privateDetails={"aadhar": result[0].aadhar ,"pan": result[0].pan, "bankdetails": result[0].bankdetails };
